...
func_4()
func_4(1,2,3,4)
func_4(1,2,3,4,5,6) # 갯수 상관없이 입력값 전부 출력
...
func_8 <- function(val,...){
  ...
  #result와 세번째 입력 값을 비교하여 작은 값을 result에 담아준다
  return(result)
}
func_8(10,3)
func_8(3,4)
...
test_ves = c(1,2,3,4)
#길이가 다른 벡터데이를 결합
cbind(students, test_ves)
data.frame(students, test_ves)
...
#---------------------------------------------------------
#새로운 학생 정보를 추가
data.frame(name='F',grade=1,gender='M',midturm=80,final=70,total=150,mean=75)->new_students
#데이터프레임 행을 추가
rbind(students,new_students)
#data.frame()함수는 행추가 X
data.frame(students,new_students)

new_student2=data.frame(name='G',gender='M',midturm=80,final=70,total=150,grade=1,mean=70)
...
#전체 학생중 중간 성적이 80점 이상이고
#기말 성적도 80이상인 학생 정보를 출력
#방법1
(students$midturm >=80)&(students$final >=80) ->flag3
students[flag3,c('midturm','final')]

#방법2
students$midturm >=80 ->flag4
students$final >=80 ->flag5
students[flag4&flag5,c('midturm','final')]
...
#order()함수에서 특정 매개변수의 인자 값을 TRUE 변경하면 내림차순 정렬
...
ifelse(
  ...
  ifelse(
    students$mean_score == 75,'hold','fail'
  )
)
