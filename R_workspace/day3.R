...
# - 절대적인 주소값
# - 환경이 변하더라도 같은 위치를 지정
# - ex) c:/users/admin/document/a.txt, htt://www.google.com
#2. 상대 경로
# - 상대적인 주소값
# - 환경이 변함에 따라 위치도 변화
# - ./:현재 작업중인 디렉토리
# - ../:현재 디렉토리에서 상이 디렉토리 이동
...
df
#절대 경로 사용
df2 <- read.csv('C:/Users/User/Documents/csv/example.csv')
df2
...
#Base
df[c(1,3)]
df[c('Name','Phone')]# 위 내용과동일한 결과 출력
...
rbind(df_1,df_3)#df_1과 df_3의 행 결합

#bind_row():데이터의 구조와 상관없이 데이터를 결합
bind_rows(df_1,df_2)
...
cbind(df,ratio)
data.frame(df,ratio)
df$ratio <-ratio #다른 ratio정보가 출력됨
head(df,1)
...
mean(df$ratio) -> mean_ratio
df <- df %>%
  mutate(group = ifelse(ratio > mean_ratio, "large", "small"))
df
#---------------------------------------------------------------
#테이블 최댓값, 최솟값안의 수량 체크
df <- df %>%
  mutate(group = ifelse(ratio > mean_ratio, "large", "small"))-> df
table(df$group)
#--------------------------------------------------------------------
midwest <- ggplot2::midwest
...
# 컬럼의 이름을 변경
midwest <- midwest %>%
  select(c("state", "popadult", "poptotal")) %>%
  rename(c("popadult" = "adult", "poptotal" = "total"))

# popadult컬럼을 adult 변경
# poptotal컬럼을 total 변경
...
midwest <- midwest %>%
  mutate(notdult_ratio = (total-adult)) -> notdult_ratio
  mutate(notdult_ratio = (total-adult/total)*100) -> notdult_ratio
midwest <- midwest %>%
  select(county, adult, total)-> notdult_ratio
#방법1--------------------답안비교----------------------
