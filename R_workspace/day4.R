...
vector_a[2] <-10 # 2번째 자리에 10대입
vector_a
vector_a[6] <-11 # 6번째 자리에 11추가
vector_a
length(vector_a) #vector_a의 길이 출력
# ifelse함수를 생성
# 매개변수 3개
...
exam %>%
  group_by(class) %>%
  summarise(mean_math = mean(math)) # 결측치가 존재하기 때문에 연산이 가능

exam %>%
  group_by(class) %>%
  summarise(mean_math = mean(math, ha.rm=T))
exam %>%
  filter(!is.na(math)) %>%
  group_by(class) %>%
...
# 제조사별 고속도로의 평균 연비가 어떠한 제조사가 가장 좋은 지
...
library(foreign)
welfare <- read.spss("./csv/Koweps/Koweps.sav",
          to.data.frame = T)
str(welfare)
View(welfare)
...
ageg_wage_gap <- ageg_income %>%
  group_by(ageg) %>%
  summarise(mean_wage = mean(income, na.rm = T))
...
left_join_data %>%
  ...
  summarise(mean_income = mean(income)) %>%
  arrange(-mean_income) %>%
  head(5) # 위 내용과 같은 값 출력 다만, 결측치 처리 방식이 다름을 인지
